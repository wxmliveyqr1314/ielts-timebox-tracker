...
describe("WallpaperSettings", () => {
  ...
  it("clicks Remove wallpaper, asserts remove not called, clicks Remove dialog button and asserts one call", () => {
    const remove = vi.fn();
    render(<WallpaperSettings wallpaper={makeWallpaper({ active: true, imageUrl: "blob", remove })} signedIn />);
  ...
  it("verifies slider min/max, changes it, and debounces setOverlayOpacity", () => {
    ...
    render(<WallpaperSettings wallpaper={makeWallpaper({ active: true, imageUrl: "blob", setOverlayOpacity })} signedIn />);
